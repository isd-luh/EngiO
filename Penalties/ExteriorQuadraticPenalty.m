...
classdef ExteriorQuadraticPenalty
    % EXTERIORQUADRATICPENALTY is a static constraint handling technique.
    % The penalty function is zero in the feasible region and and has
    % quadratic slope in the infeasible region.
    %
    % LITERATURE:
    ...
    methods
        function this = ExteriorQuadraticPenalty(r)
        ...
        function penalty = penaltyFunction(this, ~, g)
            ...
            % Return zero if feasible(g>=0), quadratic penalty if otherwise
            delta = g<0;
            penalty = this.r*(g.^2 * delta)';
        end
